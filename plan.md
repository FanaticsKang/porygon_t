...
└── reports/                  # 报告目录（自动生成）
    └── 20260303_143052_a1b2c3d/   # 按时间_commitId 命名
        ...
        └── detail/
            ├──program_<file_name>
            |  ├── test_plan_program_<file_name>.md       # 测试方案, 需要参考issues（自动生成）
...
4. 调用 Claude 生成完整测试方案，输出到 `detail/program_<file_name>/` 目录
...
5. 调用 Claude 生成针对变更的测试方案，输出到 `detail/<file_name>/` 目录
...
| 文件 | 路径 | 说明 |
|------|------|------|
| `test_plan_program_<file_name>.md` | `reports/<timestamp>_<commit_id>/detail/program_<file_name>/` | 用户指定程序的测试方案 |
| `test_plan_case_<file_name>.md` | `reports/<timestamp>_<commit_id>/detail/<file_name>/` | commit diff 文件的测试方案 |
...
### 3.1 `summary_report.md`（总体测试报告）

该文件由 Claude 基于所有文件的测试结果自动生成，汇总整体测试情况，提供项目级的质量评估。

**生成时机**：所有文件测试执行完成后，在报告阶段生成

**生成输入**：
- 所有 `program_<file_name>_summary.md` 和 `<file_name>_summary.md`
- 所有 `program_<file_name>_report.md` 和 `<file_name>_report.md`
- 覆盖率数据（来自 `fig/` 目录）
...
#### 3.1.1 执行概览
- **测试时间**：开始时间、结束时间、总耗时
- **测试范围**：测试文件总数、commit_id、分支信息
- **执行配置**：并行度、超时设置、Claude 模型配置
...
|------|------|--------|------|------|--------|------|
| `file1.py` | program | 15 | 15 | 0 | 92% | ✅ |
| `file2.py` | diff | 8 | 7 | 1 | 85% | ⚠️ |

#### 3.1.4 关键问题
- **失败用例汇总**：列出所有失败的测试，包括文件、用例名、失败原因
- **低覆盖率文件**：行覆盖率 < 90% 的文件列表及建议
- **未解决问题**：用户指定的 issues 中未被覆盖的部分

#### 3.1.5 质量评估
- **总体结论**：通过 / 有条件通过 / 未通过
- **合并建议**：针对 commit diff 场景，是否建议合并到主分支
- **风险提示**：主要风险点及缓解措施
...
```
读取所有 summary.md → 汇总统计数据 → 分析覆盖率 →
识别问题 → 生成结论 → 输出 summary_report.md
```

**依赖关系**：
- 必须在所有测试执行完成后生成
- 依赖 detail/ 目录下的所有 summary 和 report 文件