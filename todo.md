...
- **Agent Team模式**: 使用Claude Agent Teams功能，协调多个测试专家进行多角度分析

**Agent Team角色配置**:
- **用户指定程序**:
  - **功能测试专家**: 分析核心功能、正常流程和业务逻辑
  - **边界测试专家**: 专注于边界条件、极值输入和类型边界
  - **异常测试专家**: 识别错误处理、异常情况和安全风险
  - **覆盖率分析师**: 评估测试覆盖率，识别未覆盖的代码路径
- **Commit diff文件**:
  - **变更分析专家**: 分析代码变更内容、影响范围和关联关系
  - **回归测试专家**: 专注于验证原有功能未被破坏，识别潜在副作用
  - **边界测试专家**: 针对变更点分析边界条件、异常风险和安全漏洞
  - **覆盖率分析师**: 评估变更代码的覆盖率，确保新增和修改代码充分测试

**说明**：
- 此·agent·始终使用`ultrathink`模式
- 使用Claude内置Agent Teams功能实现多角色协作
...
**输出**:
- `test_<file>.py` - 生成或更新的测试代码（支持`--summary`选项，用于生成报告；日常跑测试用例时不使用该选项）
